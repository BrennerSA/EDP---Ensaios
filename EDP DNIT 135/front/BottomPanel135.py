...
class BottomPanel(wx.Panel):
        ...
        def __init__(self,main, parent, NomeEnsaio, top):
            ...
            self.onlyMR=False
            self.tens=0
            self.tensao=float(self.ensaio[26])
            if self.resistencia==0:
                self.resistencia=0.05
            else:
                Forca=(self.resistencia*float(self.ensaio[26])*3.1416*self.ensaio[14]*self.ensaio[15]*0.000001)/2
                tens=Forca*0.1618*1000
                self.TopPanel.PressaoAtualGolpe=tens
    ...
    #--------------------------------------------------
        ...
        def CONDIC(self, event):
            ...
            if self.condRT:
                ...
                tens=Forca*0.1618*1000
                self.tens=tens
                self.SigmaAlvo.SetLabelText("%.3f" % (tens))
            ...
            message2 = "Verifique o sensor de deslocamento e o sensor de fim de ensaio."
